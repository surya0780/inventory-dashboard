...
function lookupProduct() {
  ...
  const item = inventoryData.find(i => i.item_id.toLowerCase() === input.toLowerCase());
  ...
  result.innerHTML = `
    <div class="lr-product">
      ...
      <div class="lr-details">
        <div class="lr-field"><span class="lr-label">Item ID</span><span class="lr-value">${item.item_id}</span></div>
        <div class="lr-field"><span class="lr-label">UPC</span><span class="lr-value">${item.upc}</span></div>
...
function populateFilters() {
  const brands = [...new Set(inventoryData.map(i => i.brand))].sort();
  const cats = [...new Set(inventoryData.map(i => i.category))].sort();
...
function viewHistory(id) {
  ...
  body.innerHTML = `<h4 style="margin-bottom:16px">${item.product_name}</h4><div class="timeline">${dates.map((d, i) =>
    `<div class="timeline-item"><div class="tl-date">${d.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</div><div class="tl-text">${['Stock updated to ' + (item.qty + i * 5), 'Price adjusted to ₹' + (item.mrp - i * 2), 'Threshold changed to ' + item.threshold, 'Stock replenished +' + (10 + i * 3), 'Initial stock entry'][i]}</div></div>`
  ).join('')}</div>`;
...
function exportCSV() {
  ...
  const rows = filteredData.map((item, i) => [i + 1, item.item_id, item.upc, `"${item.product_name}"`, item.brand, item.category, item.mrp, item.qty, item.threshold, item.uom, getStatus(item).label, item.last_updated]);
...
function generateNotifications() {
  const lowItems = inventoryData.filter(i => Number(i.qty) > 0 && Number(i.qty) < Number(i.threshold));
  const outItems = inventoryData.filter(i => Number(i.qty) === 0);
...
function initCharts() {
  ...
  // Brand chart
  const brandMap = {};
  inventoryData.forEach(i => { brandMap[i.brand] = (brandMap[i.brand] || 0) + Number(i.qty); });
  ...
  // Low stock chart
  const lowItems = inventoryData.filter(i => Number(i.qty) > 0 && Number(i.qty) < Number(i.threshold)).slice(0, 10);
  ...
  // Value chart
  const catValue = {};
  inventoryData.forEach(i => { catValue[i.category] = (catValue[i.category] || 0) + Number(i.mrp) * Number(i.qty); });
